...
def load_query(query_name):
    try:
        query_filename = os.path.join('..', 'sql', str(query_name) + '.sql')
        with open(query_filename, 'r', encoding='utf-8') as query_file:
            return query_file.read()
    except Exception as e:
        click.echo("Unable to load query file sql/{}".format(query_filename))
